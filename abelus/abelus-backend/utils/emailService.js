...
const sendEmail = async ({ to, subject, text, html, headers }) => {
    try {
        ...
        // Ensure "Abelus" is always in the From field
        let from = process.env.SMTP_FROM || "Abelus <noreply@abelus.rw>";
        if (!from.includes("Abelus") && !from.includes('"')) {
            from = `"Abelus" <${process.env.SMTP_USER || from}>`;
        }
...
export const sendSellerApprovedEmail = async (seller) => {
    ...
    <div style="font-family: 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <p style="color: #6b7280; margin-top: 24px;">Welcome to the Abelus family!</p>
    </div>`;
...
export const sendNewOrderEmail = async (seller, order) => {
    ...
    return sendEmail({
        to: seller.email,
        subject: `🛒 New Order: ${order.orderNumber || order._id} - Abelus`,
        html
    });
};
...
export const sendWelcomeEmail = async (email) => {
    ...
    const headers = {
        'List-Unsubscribe': `<${unsubscribeUrl}>, <mailto:${process.env.SMTP_USER || 'noreply@abelus.rw'}?subject=unsubscribe>`,
    ...
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.05); border: 1px solid #e5e7eb;">
        ...
        <div style="background: linear-gradient(135deg, #6366f1 0%, #4f46e5 100%); padding: 32px 24px; text-align: center;">
            <h1 style="color: #ffffff; margin: 0; font-size: 28px; font-weight: 800; letter-spacing: -0.5px;">Welcome to Abelus! 🎉</h1>
        </div>
        ...
        <div style="padding: 32px 24px;">
            ...
            <p style="color: #374151; font-size: 16px; line-height: 1.6; margin-bottom: 24px;">
                Thank you so much for subscribing to the Abelus newsletter! We're thrilled to have you join our community of creative minds and printing enthusiasts.
            </p>
        ...
        <div style="background-color: #f9fafb; padding: 24px; text-align: center; border-top: 1px solid #e5e7eb;">
            <p style="color: #9ca3af; font-size: 13px; margin: 0;">
                © ${new Date().getFullYear()} Abelus. All rights reserved.<br>
